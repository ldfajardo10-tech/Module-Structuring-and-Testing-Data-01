...
function getCardValue(card) {
  const rank = card.substring(0, card.length - 1); //it takes the first index so all cards with the same group, but different sign would have the same value. Example: J♠ and Q♥ = 10.
...
function assertThrows(func, errorMessage) {
  ...
  const passed = thrownError && thrownError.message === errorMessage;

  console.assert(
    passed,
    `Expected error "${errorMessage}", but got ${
